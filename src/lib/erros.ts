...
export function isErroConexao(erro: unknown): boolean {
  ...
  const padrao = /failed to fetch|network\s?error|load failed|offline|sem conex|conexão|timeout|abort/i;
  return padrao.test(msg);
}

export function formatarMensagemErro(erro: unknown, fallback: string = "Ocorreu um erro inesperado. Tente novamente."): string {
