...
`handleQuitar`/`handleQuitarTodas` exibem "Lançamento marcado como quitado" **antes** do `await quitarDivida(...)`; em falha o usuário recebe toast de sucesso seguido de erro.
...
O handler de teclado está preso ao `<ul tabIndex={-1}>`, mas o foco nunca sai do botão gatilho — ArrowUp/Down não fazem nada e Enter **fecha** o popup. Usuário de teclado abre mas não consegue escolher.
**Refatoração**: mover o handler para o `div` contêiner, ou focar o `ul` ao abrir (`listaRef.current?.focus()`), ou usar `aria-activedescendant`. Corrigir junto com P2-10 (mesma extração).
